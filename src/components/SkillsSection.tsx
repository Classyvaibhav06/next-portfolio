...
import { Zap, Code, Atom, Server, Globe, Palette, Database, Layout, Star, Shield, Medal } from "lucide-react";
...
const skills: SkillData[] = [
  ...
  { name: "HTML5", icon: Globe, rank: "gold", power: 95, description: "Semantic markup & accessibility" },
  { name: "Tailwind CSS", icon: Palette, rank: "gold", power: 90, description: "Utility-first CSS framework" },
  { name: "MongoDB", icon: Database, rank: "bronze", power: 80, description: "NoSQL database management" },
  { name: "Bootstrap", icon: Layout, rank: "silver", power: 85, description: "Responsive web design" },
];
      ...
      <div className="max-w-7xl mx-auto" ref={sectionRef}>
        ...
        >
          {skills.map((skill) => {
            ...
            const RankIcon = { gold: Star, silver: Shield, bronze: Medal }[skill.rank];
            return (
              <div key={skill.name} className={`skill-card p-5 relative overflow-hidden ${skill.rank}`}>
                <div className={`absolute top-3 right-3 rank-badge ${rankClasses[skill.rank]} flex items-center gap-1`}>
                  <RankIcon size={10} /> {rankLabels[skill.rank]}
                </div>
                <div className="skill-icon mb-4">
                  <Icon size={24} className={textClasses[skill.rank]} />
                </div>
